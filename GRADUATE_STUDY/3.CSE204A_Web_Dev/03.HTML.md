...
Used for abbreviations and acronyms. The "title" attribute on the opening tag is used to specify the full term I.e. `<abbr title="Hypertext Markup Language">HTML</abbr> is the best!` On hover, the browser will show the full term in a tooltip

------

### Citations: `<cite>`

Used to list a reference. Browsers render the cite element in italics.
...
`<address>` can be used to wrap contact info for an article.
...
- Usually styled as strikethrough
...
- ```
  ...
  attribute provides a text description of the image (always use this).
...
- ```
  ...
  creates a table cell within the current row that contains a row or column heading
...
- ```
  ...
  creates a table cell within the current row

  - A row can have mutlipleelements to create columns
...
Used to wrap navigation elements * Like a sematic `div`.
